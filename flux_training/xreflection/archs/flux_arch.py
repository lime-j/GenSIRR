...
# from diffusers.loaders import FluxLoraLoaderMixin, TextualInversionLoaderMixin, FluxIPAdapterMixin
...
# from peft import get_peft_model, LoraConfig, train_model, merge_and_unload
# from diffusers.loaders imp
...
DEFAULT_EMBEDDING_DIR = REPO_ROOT / "demo"


def _resolve_model_path(path_or_repo_id: str) -> str:
    """Resolve local paths while allowing Hugging Face repo ids."""
    candidate = Path(path_or_repo_id).expanduser()
    looks_like_local_path = (
        path_or_repo_id.startswith(("/", ".", "~"))
        or candidate.exists()
    )
...
class FluxKontext(nn.Module):
    _callback_tensor_inputs = ["latents", "prompt_embeds"]

    def __init__(self, model_path, train_dit=True, vae_path=None):
        super().__init__()
        ...
        vae_path = vae_path or os.environ.get("FLUX_VAE_PATH") or os.environ.get("VAE_PATH")
        if vae_path is None:
            raise ValueError(
                "FluxKontext requires a merged VAE path. Set network_g.vae_path "
                "in the config or export FLUX_VAE_PATH."
            )
        from diffusers import AutoencoderKL
        self.vae = AutoencoderKL.from_pretrained(
            _resolve_model_path(vae_path),
        ...
        # Add PiSSA-initialized LoRA to the transformer for training
        # We wrap the transformer with PEFT LoRA using PiSSA fast init if available.
        # try:
        #     if self.train_dit:
        #         # Target common linear submodules in Flux/DiT blocks
        #         target_modules = [
        ...
        #             r=512,
        #             lora_alpha=512,
        #             lora_dropout=0.0,  # PiSSA uses principal components; keep dropout 0
        #             target_modules=target_modules,
        #             bias="none",
        #         )

        #         # Prefer PiSSA fast init when PEFT supports it
        #         try:
        #             lora_cfg = LoraConfig(init_lora_weights="pissa_niter_4", **lora_kwargs)
        #         except TypeError:
        #             # Older PEFT without PiSSA support
        #             lora_cfg = LoraConfig(**lora_kwargs)
        #             logger.warning(
        #                 "PEFT does not support PiSSA 'init_lora_weights'; falling back to default init."
        #             )

        #         self.transformer = get_peft_model(self.transformer, lora_cfg)
        #         # Make sure LoRA adapters are trainable and base is frozen
        #         try:
        #             self.transformer.print_trainable_parameters()
        #         except Exception:
        #             pass
        # except Exception as e:
        #     logger.warning(f"Failed to attach PiSSA LoRA to transformer: {e}")

        embedding_dir = Path(os.environ.get("GENSIRR_EMBEDDING_DIR", DEFAULT_EMBEDDING_DIR)).expanduser()
        self.cached_prompt_embeds = torch.nn.Parameter(torch.load(embedding_dir / "prompt_embeds.pth", map_location='cpu'))
        self.cached_pooled_prompt_embeds = torch.nn.Parameter(torch.load(embedding_dir / "pooled_prompt_embeds.pth", map_location='cpu'))
        self.cached_text_ids = torch.nn.Parameter(torch.load(embedding_dir / "text_ids.pth", map_location='cpu'))
    @staticmethod
    ...
    def forward(
        ...
        **kwargs
        ...
        #     max_sequence_length=max_sequence_length
        # )
        # pooled_prompt_embeds = self.de_model(image)
        if image is not None and not (isinstance(image, torch.Tensor) and image.size(1) == self.latent_channels):
            img = image[0] if isinstance(image, list) else image
            image_height, image_width = image.shape[2], image.shape[3]
            #aspect_ratio = image_width / image_height
            #if _auto_resize:
            #    _, image_width, image_height = min((abs(aspect_ratio - w / h), w, h) for w, h in PREFERRED_KONTEXT_RESOLUTIONS)
    ...
    def training_step(self, image, target, prompt=None, prompt_2=None, generator=None, cached_prompt=False):
        device = self.text_encoder.device
        if self.text_encoder_2 is not None:
            self.text_encoder.max_position_embeddings = 77
            self.text_encoder_2.max_position_embeddings = 512
        ...
        with torch.no_grad():
            condition_image_latents, lq_image_ids = self.encode_image(image)
            x_0, target_image_ids = self.encode_image(target)
            ...
            if self.transformer.config.guidance_embeds:
                ...
                guidance = guidance.expand(x_0.shape[0])
            else:
                guidance = None
            
            # if (image.shape[0] > 1):
            #     prompt_embeds = torch.cat([prompt_embeds] * image.shape[0], dim=0)
            #     pooled_prompt_embeds = torch.cat([pooled_prompt_embeds] * image.shape[0], dim=0)
            #     text_ids = torch.cat([text_ids] * image.shape[0], dim=0)

            t = torch.sigmoid(torch.randn((x_0.shape[0],), device=device)) #* 0.5
            x_1 = torch.randn_like(x_0, device=device)
            t_ = t.unsqueeze(1).unsqueeze(1)

            x_t = ((1 - t_) * x_0 + t_ * x_1).to(self.dtype)
            
            latent_model_input = torch.cat([x_t, condition_image_latents], dim=1)
            img_ids = torch.cat([lq_image_ids, target_image_ids], dim=0)
        ...
        model_pred = self.transformer(
            ...
            txt_ids=text_ids, img_ids=img_ids, return_dict=False, guidance=guidance
        )[0][:, :x_0.size(1)]

        return torch.nn.functional.mse_loss(model_pred, (x_1 - x_0), reduction="mean")

    def get_dit_params(self):
        # Train only LoRA parameters when LoRA is enabled; otherwise, nothing
        params = []
        try:
