...
class DSRTestDataset(BaseDataset):
    ...
    def align(self, x1, x2):
        # First, if the short side is greater than 1024, resize so short side becomes 1024
        h, w = x1.height, x1.width
        short_side = min(h, w)
        if short_side > 1024:
            if h <= w:
                x1 = scale_height(x1, 1024)
                x2 = scale_height(x2, 1024)
            else:
                x1 = scale_width(x1, 1024)
                x2 = scale_width(x2, 1024)
            h, w = x1.height, x1.width

        # Then align to be divisible by the round factor
    ...
    def __getitem__(self, index):
        ...
        # if gt is not png, then use jpg
        if os.path.exists(join(self.datadir, 'transmission_layer', fn)):
            t_img = Image.open(join(self.datadir, 'transmission_layer', fn)).convert('RGB')
        else:
            if os.path.exists(join(self.datadir, 'transmission_layer', fn.replace('.jpg', '.png'))):
                t_img = Image.open(join(self.datadir, 'transmission_layer', fn.replace('.jpg', '.png'))).convert('RGB')
            else:
                t_img = Image.open(join(self.datadir, 'transmission_layer', fn.replace('.png', '.jpg'))).convert('RGB')
    ...
    def align(self, x1, x2, x3):
        ...
        if short_side > 1024:
            if h <= w:
                x1 = scale_height(x1, 1024)
                x2 = scale_height(x2, 1024)
                x3 = scale_height(x3, 1024)
            else:
                x1 = scale_width(x1, 1024)
                x2 = scale_width(x2, 1024)
                x3 = scale_width(x3, 1024)
            h, w = x1.height, x1.width

        # Then align to be divisible by 32
        h, w = h // 32 * 32, w // 32 * 32
        x1 = x1.resize((w, h))
        x2 = x2.resize((w, h))
        x3 = x3.resize((w, h))
...
class RealDataset(BaseDataset):
    ...
    def align(self, x):
        ...
        if short_side > 1024:
            if h <= w:
                x = scale_height(x, 1024)
            else:
                x = scale_width(x, 1024)
            h, w = x.height, x.width
